...
class MemeStore {
	...
	setPostMemeAPIResponse(
		response: PostMemeResponse | null,
		requestData: PostMemeRequest
	): void {
		if (response) {
			const { id } = response;
	...
	@action.bound
	setGetMemesAPIResponse(response: GetMemesResponse | null): void {
		if (response) {
			const { total: totalMemes, memes } = response;
	...
	@action.bound
	setGetMemeAPIResponse(response: GetMemeResponse | null): void {
		if (response) {
			const { id, name, caption, url } = response;
			this.selectedMeme = new MemeModel({ id, name, caption, url });
		}
	}
	...
	@action.bound
	setEditMemeAPIResponse(id: string, updatedMemeData: EditMemeRequest): void {
